...
func (nr *NotificationRoutes) RegisterRoutes(api *gin.RouterGroup) {
	...
	notifications.Use(middleware.Auth(nr.tokenService))
	{
		...
		notifications.POST("/unreadCount", nr.notificationHandler.GetUnreadCount)       // 未读数
		notifications.POST("/read", nr.notificationHandler.MarkNotificationRead)        // 标记单条已读
		notifications.POST("/readAll", nr.notificationHandler.MarkAllNotificationsRead) // 标记全部已读
	}
}
