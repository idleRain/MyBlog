...
import { cn, type WithElementRef, type WithoutChildren } from '@myblog/shared'
...
const formattedLabel = $derived.by(() => {
  ...
  const [firstSeries] = visibleSeries
  // 项级图表在仅悬浮项有值的场景下队列可能为空，此时不存在可取的系列引用。
  const item: TooltipPayload | undefined = firstSeries
